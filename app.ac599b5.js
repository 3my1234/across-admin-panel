...
$("clearAdminNotifications")?.addEventListener("click", () => {
  state.activityItems = [];
...
function activityStorageKey() {
  return `across.admin.activityCursor.${state.adminId || "unknown"}`;
}

function startAdminActivityPolling() {
  stopAdminActivityPolling();
  state.activityItems = [];
  state.activityCursor = localStorage.getItem(activityStorageKey()) || "";
  state.activitySeeded = Boolean(state.activityCursor);
  renderAdminNotifications();
...
async function pollAdminActivity() {
  ...
  try {
    ...
    const wasSeeded = state.activitySeeded;
    if (result.cursor) {
      state.activityCursor = result.cursor;
      state.activitySeeded = true;
      localStorage.setItem(activityStorageKey(), result.cursor);
    }
    if (!wasSeeded) return;
...
function renderAdminNotifications() {
  ...
  if (badge) {
    badge.textContent = String(Math.min(state.activityItems.length, 99));
    badge.classList.toggle("hidden", state.activityItems.length === 0);
  }
  ...
  if (!state.activityItems.length) {
    ...
    empty.textContent = "No new operational updates.";
  ...
  state.activityItems.forEach((item) => {
    ...
    button.addEventListener("click", async () => {
      state.activityItems = state.activityItems.filter((entry) => entry.id !== item.id);
      renderAdminNotifications();
